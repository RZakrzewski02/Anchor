...
export default function ChatWindow({ 
  ...
  // Pobieramy status dostępności w czasie rzeczywistym
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50">
      {/* Nagłówek czatu */}
      <div className="p-3 border-b border-slate-200 flex items-center gap-3 bg-white shadow-sm z-10 shrink-0">
        <div className="w-8 h-8 rounded-full overflow-hidden border border-slate-100 bg-slate-100 flex items-center justify-center text-slate-400">
          {friendAvatar ? (
            <img src={friendAvatar} className="w-full h-full object-cover" alt={friendName} />
          ) : (
            /* ZMIANA: Ikona ludka zamiast pierwszej litery */
            <User size={16} />
          )}
        </div>
        <div>
          <h2 className="font-bold text-slate-900 text-sm">{friendName}</h2>
          <div className="flex items-center gap-1.5">
             {/* DYNAMICZNY STATUS: Zmienia kolor i tekst zależnie od obecności */}
      ...
      <div className="p-3 bg-white border-t border-slate-200 shrink-0">
        <form onSubmit={handleSend} className="flex gap-2">
          <input 
            type="text" 
            className="flex-1 text-slate-900 bg-white border border-slate-300 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 rounded-lg px-4 py-2.5 outline-none transition-all text-sm placeholder:text-slate-400"
